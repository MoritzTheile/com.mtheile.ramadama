...
export class AuthServerProvider {
    ...
    login(credentials): Observable<any> {
        const data = 'username=' +  encodeURIComponent(credentials.username) + '&password=' +
            ...
            'client_secret=my-secret-token-to-change-in-production&client_id=ramadamaapp';
        const headers = new Headers ({
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json',
            'Authorization': 'Basic ' + this.base64.encode('ramadamaapp' + ':' + 'my-secret-token-to-change-in-production')
        });
